...
function moveEverything(){
    ...
    if(ballX < 0){
       if(ballY > paddle1Y && ballY < paddle1Y+PADDLE_HEIGHT){
           ballSpeedX = -ballSpeedX;
            ...
            player2Score++; //must be before ball resets
            ballReset();
    ...
    if(ballX > canvas.width){
        if(ballY > paddle2Y && ballY < paddle2Y+PADDLE_HEIGHT){
            ...
            player1Score++; //must be before ball resets
            ballReset();
...
function colorRect(leftX,topY,width, hieght,drawColor){
    canvasContext.fillStyle = drawColor;
    canvasContext.fillRect(leftX,topY,width, hieght);
}
